...
|---|---|---|---|---|
| Entry point | `SKILL.md`: routing, ten core rules, intake, the run loop. No craft values | 207 lines | It absorbs the manual. Every new unit wants four lines here | A line budget with a ceiling *and* a floor, and a written reason beside every raise |
...
| Instruments | `scripts/`: arithmetic and gates, each returning a verdict rather than an opinion | 49 tools | It returns advice. Advice cannot be wrong, so it cannot be fixed | `--self-check` on every tool that computes, verified against known inputs |
...
|---|---|---|
| `source` / `source_url` / `retrieved` | 20 of 35 tables | The page and the day somebody read it. A spec with no retrieval date is a spec with no expiry |
...
| `what_it_does_not_establish` | 11 tables carry a limit | The load-bearing one. A benchmark quoted without its limits is how a plan acquires a number nobody can defend |
...
The architecture is not a description. It is 635 tests, and these are the ones that hold the shape
