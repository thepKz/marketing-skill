...
|---|---|---|---|---|
| Entry point | `SKILL.md`: routing, ten core rules, intake, the run loop. No craft values | 203 lines | It absorbs the manual. Every new unit wants four lines here | A line budget with a ceiling *and* a floor, and a written reason beside every raise |
...
| Instruments | `scripts/`: arithmetic and gates, each returning a verdict rather than an opinion | 48 tools | It returns advice. Advice cannot be wrong, so it cannot be fixed | `--self-check` on every tool that computes, verified against known inputs |
