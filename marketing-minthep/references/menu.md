# Menu: engineering, design, art direction, brand vision

One artifact, one owner. Marketing locks the items, prices, and economics; the same hands then
commit to one visual direction and art-direct inside it. Everything menu lives in this file.

## Contents

- Decision contract (what a menu run needs before design)
- Menu design and engineering (economics, decision order, typography, print, production method)
- Menu art direction (direction, focal mechanism, text ownership)
- Brand-led menu vision (recognition systems for redesigns)
- Deep dossier: Kasavana & Smith arithmetic, evidence audit, worked bún bò directions


For a dish photo or restaurant brief, weigh the three families — `classic/local`, `modern editorial`, `high-volume readable` — against audience, venue, price point, print/digital surface, and service speed, then commit to one. Do not present the families to the user as options; the choice is the model's, justified in one line of the handoff.

The full menu research, information architecture, typography, print, accessibility, and production method follow in the sections below. This opening block is the short decision contract used by the pipeline registry.

## Content model

Capture dish name, pronunciation if needed, ingredients confirmed, allergens, portion, price, modifiers, dietary labels, availability, and one truthful sensory description. Never infer ingredients or prices from a photo.

## Layout brief

Define paper/screen size, grid, section hierarchy, type scale, price alignment, image ratio, whitespace, contrast, bilingual order, and a text-safe area. Produce a low-fidelity wireframe before a visual prompt. Keep each item scannable in under two seconds and test the longest real item name.

## Visual direction

For food images specify hero angle (usually 30-45 degrees or overhead), lens feel, steam/texture only when truthful, controlled highlights, contact shadow, garnish limits, background color, and crop variants. Use the same visual grammar across the menu, social post, and delivery thumbnail.

## Outputs

Return three option cards, a recommended direction with rationale, a wireframe, copy pack, image prompt(s), print/digital specs, and a QA checklist for price, allergens, legibility, bleed, and export.
...
Use this reference for printed menus, menu boards, delivery listings, QR menus, restaurant landing pages, and food-brand concept directions. Do not begin with visual style; begin with item economics, customer decision order, operational truth, and the ordering environment. The detailed evidence audit and worked Vietnamese example live in the deep dossier section at the end of this file.

## Required inputs

- Item name, category, price, portion, ingredients, allergens/dietary markers, and availability.
- Plate cost and units sold when menu engineering is requested.
- Channel: table menu, counter board, delivery app, QR/mobile, poster, or social post.
- Brand position, service speed, daypart, audience, language, and photography assets.
- Printing size/material or digital viewport.

Never invent prices, ingredients, provenance, dietary claims, awards, spice levels, or availability.

## Engineering before decoration

Use contribution margin in currency, not margin percentage:

```text
contribution margin = selling price - plate cost
menu mix % = item units / category units * 100
popularity threshold = (1 / number of comparable items) * 0.70
weighted average CM = sum(item CM * item units) / total category units
```

Run separate analyses for mains, add-ons, drinks, desserts, and channel-only items. Mixing them can silently change classifications. Label the method and threshold. Treat Stars, Plowhorses, Puzzles, and Dogs as hypotheses for pricing, placement, naming, bundling, or removal; protect dietary-gate items and operational necessities.

## Information architecture

Design for sequential scanning, not the disproven restaurant `golden triangle` myth.

1. Orient: restaurant name, service proposition, language, ordering instructions.
2. Decide: categories in natural meal order; hero or recommended item near the start of its category.
3. Compare: consistent item name, short sensory/provenance descriptor, portion, and price alignment.
4. Complete the basket: add-ons, beverages, combos, and desserts near the point where they become relevant.
5. Reassure: spice, dietary, allergen, service charge, availability, and contact details.

Keep categories distinct through spacing and type hierarchy. Do not use boxes, photos, badges, color, icons, and oversized type on the same item. One emphasis device per priority is usually enough.

## Three direction families

### Modern editorial

- Strong typographic hierarchy, generous spacing, limited photography, asymmetrical but disciplined grid.
- Best for focused menus, premium casual concepts, or a signature dish.
- Risk: excessive whitespace can reduce scannability and item density on small formats.

### Warm vernacular

- Local materials, warm neutrals, hand-made cues, documentary food photography, culturally specific language.
- Best when place, craft, family recipe, or street-food authenticity is central.
- Risk: decorative nostalgia can become illegible or stereotyped.

### Fast-service modular

- Large price and item name, clear category blocks, combo logic, high-distance readability, minimal copy.
- Best for counter boards, kiosks, food courts, delivery thumbnails, and high-throughput ordering.
- Risk: visual noise and promotion overload.
...
- Overhead menu-recognition image with bowl geometry and toppings clear.
- Three-quarter appetite hero with broth depth, steam, sheen, and edible entry point.
- Macro proof image for noodles, meat, herbs, broth, or preparation.
- Scale/context image with hand, tableware, or place setting.
- Delivery/listing cutout with clean silhouette and truthful portion.

Preserve real serving size, ingredient truth, bowl contact, heat behavior, and garnish logic. Avoid ingredient explosions, impossible steam, duplicated toppings, and over-saturated reds/greens.

## Wireframe contract

```text
CANVAS / VIEWPORT
READING DISTANCE AND ORDERING CONTEXT
HEADER / ORIENTATION
CATEGORY ORDER
ITEM ROW: name -> descriptor -> portion/marker -> price
HERO EMPHASIS DEVICE
ADD-ON / COMBO LOCATION
FOOTER / LEGAL / ALLERGEN / SERVICE INFORMATION
PHOTO SLOTS AND CROP
TYPE SIZES, SPACING, GRID, COLOR ROLES
```

## Verification

- Recalculate every price and contribution margin from source data.
- Read at final physical size or actual phone viewport.
- Check contrast, Vietnamese diacritics, currency formatting, and line wrapping.
- Confirm all food photos match the served product.
- Export print work with the printer's bleed, profile, and finishing requirements; verify live platform requirements for delivery apps.
...
For a redesign, restaurant identity, or "make this more branded" request, read the brand-led menu vision section below before choosing a visual route. Use it to compare brand systems, define recognition, and avoid palette-only or ornament-only art direction.
...
# Brand-Led Menu Vision

Use this reference when redesigning a menu, building restaurant recognition, or diagnosing a menu that is clean but forgettable. Study systems, not surfaces. Never copy a named brand's layout, logo, mascot, illustration style, or proprietary device.

## Research record — 2026-08-04

| Brand/source | Observed system | Useful principle | Do not copy |
|---|---|---|---|
| Shake Shack — [Pentagram](https://www.pentagram.com/work/shake-shack) | Neon-like line icons, a distinctive typographic voice, simple packaging, and a menu board described as a consistent graphic focus across locations | Make the menu a repeated brand anchor; use one flexible graphic system across environments | Curvy menu icons, exact lettering, green/black palette |
| Sweetgreen — [COLLINS](https://wearecollins.com/case-studies/sweetgreen/) and [It's Nice That](https://www.itsnicethat.com/news/collins-sweetgreen-graphic-design-080621) | Custom type, mid-century cookbook/gardening illustration, food photography, expanded produce-led color, humor, and a system spanning signage, digital, packaging, uniforms, and stores | Brand memory comes from coordinated typography, image behavior, color, and tone; ingredients can become characters without losing food truth | SweetSans/Grenette, oversized surreal produce, exact cookbook illustration language |
| 4P's — [Base Design](https://www.basedesign.com/projects/4ps) | A master idea, “Platforms for Peace,” expressed through a warm but precise modular identity across menus, packaging, uniforms, signage, and environments; a mascot adds humanity | Begin with a brand premise that can govern every touchpoint; modularity must preserve warmth and recognition | BUU mascot, 4P's wording, exact Japanese/Vietnamese visual balance |
| Dishoom — [&Smith](https://andsmithdesign.com/brands/dishoom) | Thin papers, fast printing, multiple logo behaviors, typographic source research, and location-specific stories form a believable Bombay cafe world | A menu can act as an in-world object; material, printing logic, voice, and imperfection can carry identity more strongly than decoration | Bombay/Irani cafe nostalgia, exact ephemera, logos, stories, or typography |
| Highlands Coffee — [official product architecture](https://www.highlandscoffee.com.vn/vn/san-pham.html) | Strong category families, one image-led entry per family, Vietnamese ingredient/origin story, stable red-brown recognition, and direct “explore/order” behavior | Ordinary chains win through repeatable category architecture and color commitment, not novelty on every item | Highlands red, logo, category naming, product photos |
| Jollibee Vietnam — [official menu](https://jollibee.com.vn/mon-moi-mon-ngon.html) | Bestseller-first taxonomy, photo-led combo cards, descriptive contents, prominent price, strong red/yellow identity, and immediate cart behavior | Commercial menus must help diners choose portions and bundles quickly; recognition and conversion can coexist | Mascot, red/yellow trade dress, combo names, card styling |
| Din Tai Fung public menu boards and location menus — secondary visual observation | White field, strict red header band, repeatable dish-photo modules, stable category markers, and restrained supporting graphics | A chain can use many food images without visual chaos when one rigid brand band and one module grammar control them | Red-band trade dress, exact grid, calligraphy, logo |

## Cross-brand findings

1. **The menu is a brand touchpoint, not a poster.** Its visual grammar must also work on a sign, takeaway label, QR header, staff card, or social crop.
2. **Recognition comes from a repeated device.** Strong brands repeat one useful asset: a type voice, category rail, icon grammar, material, illustration behavior, color field, or ordering module.
3. **A palette is not an identity.** “Cream + cobalt + orange” remains generic until each color has a stable role and a repeatable shape, position, or behavior.
4. **Photography follows a system.** Images share crop logic, camera family, scale hierarchy, light, surface, and narrative role. They are not independent attractive pictures.
5. **Ordinary brands are often commercially clearer.** Bestseller-first grouping, combo logic, visible price, portion explanation, and stable product families reduce decision time.
6. **Premium brands remove noise, not information.** They use material, typography, and confident editing; they do not create arbitrary empty space.
...
- **Brand premise:** one sentence a restaurant employee could explain without design vocabulary.
- **Signature device:** one repeated visual behavior with a functional job.
- **Menu architecture:** category, hero, price, descriptor, add-on, and ordering logic.
- **Image behavior:** continuous scene, disciplined modules, ingredient atlas, documentary crop, or another named system.
- **Material behavior:** what the menu appears printed, painted, stamped, tiled, written, or displayed on, and why.
- **Extension test:** how the signature appears on a takeaway sticker, QR landing header, menu board, and social post.

Fail `PALETTE-ONLY` when recognition depends only on color. Fail `MOTIF-ONLY` when decoration disappears and nothing branded remains. Fail `ONE-OFF` when the design cannot add a fifth category without being recomposed from scratch.

Run the three-second cover test: hide the logo and restaurant name. A viewer should still identify the same brand world from the category device, image behavior, material, typography, and color roles. If not, strengthen the system rather than adding ornament.

## Image and layout families

Choose one family deliberately:

1. **Continuous table scene:** several dishes share one physical surface, light, perspective, and contact-shadow field. Best when abundance or a meal ritual is the promise.
2. **Disciplined product modules:** consistent crops and labels under one strong brand band. Best for chains, menu boards, delivery, and many SKUs.
3. **Ingredient or process atlas:** food, raw ingredients, tools, and preparation evidence form a system. Best when sourcing or craft is the proof.
4. **Story ephemera:** the menu behaves like an object from the brand's world: ledger, ticket, newspaper, field note, ration card, or workshop sheet. Best when the restaurant has a real narrative premise.
...
<!-- Deep dossier merged from references/dossiers/menu-design-and-engineering.md (2026-08-06). External facts retrieved 2026-07-29. -->
