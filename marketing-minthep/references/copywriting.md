...
Every content item must connect:

```text
audience state -> question/tension -> useful promise -> evidence -> format -> channel -> next action
```

Do not begin with a list of generic topics. Define the change in understanding or behavior first.

### Pillar construction

Create three to five pillars. Each pillar needs:

- Audience problem or ambition.
- Product or brand authority to speak.
- Repeatable evidence source.
- Funnel role: discover, trust, convert, or retain.
- Distinctive point of view.
- Boundaries: what the pillar will not cover.

Useful pillar roles:

1. **Problem language**: help the audience name and diagnose the issue.
2. **Mechanism education**: explain how the solution works.
3. **Proof in practice**: demonstrate outcomes, process, or customer evidence.
4. **Decision support**: comparisons, objections, costs, risks, and selection criteria.
5. **Use and expansion**: onboarding, workflows, advanced use, and retention.

### Content brief

For each asset define:

- Working title and search/social hook.
- Target audience state and single reader question.
- One-sentence takeaway.
- Original angle and saturated angle to reject.
- Evidence, examples, sources, and product truth required.
- Structure or beat sheet.
- Primary CTA and natural next asset.
- Channel, format, length, owner, due date, and reuse potential.
- Claim and rights risks.

For SEO work, verify live search intent and current results. Do not infer keyword volume or ranking difficulty without data.

### Repurposing graph

Repurpose the idea, not the file:

- Research or customer insight -> long-form source asset.
- Source asset -> social point-of-view posts, carousel, short video script, email, sales snippet.
- Proof fragment -> ad or landing-page module.
- Audience question -> FAQ, comparison, onboarding, or support content.

Adapt hook, pacing, evidence density, CTA, and visual grammar for each channel. Do not paste the same copy everywhere.

### Editorial cadence

Prefer a sustainable operating loop:

1. Capture audience questions and product evidence.
2. Prioritize by strategic value, demand signal, and production cost.
3. Brief and produce.
4. Review facts, claims, voice, and channel fit.
5. Publish only when explicitly authorized.
6. Distribute and repurpose.
7. Measure against the intended behavior.
8. Refresh, consolidate, or retire based on evidence.

### Quality gate

Reject content that:

- Could be published by any competitor after changing the logo.
- Makes a promise without mechanism or evidence.
- Uses a trend as the entire idea.
- Adds length without increasing clarity or value.
- Hides the product relationship until an arbitrary final CTA.
- Invents statistics, quotations, customer stories, or search data.
- Optimizes a headline for clicks while violating the content promise.
...
Gen Z slang is a register, not a dictionary. The mistake that produces cringe is treating
`data/genz-lexicon.csv` as a thesaurus — writing a normal corporate sentence and swapping three words
for slang. Real usage works the other way: the whole utterance is casual (short clauses, no formal
pronouns, punctuation relaxed, emotion allowed to leak), and the slang terms sit inside that register
naturally. One slang word in a stiff sentence reads as a 40-year-old brand manager; a fully casual
sentence with zero slang words reads fine. If the draft cannot commit to the register, use no slang
at all.

Search the lexicon by either direction — the word you met or the meaning you need:

```
python scripts/find_recipe.py --table genz --query "khum"
python scripts/find_recipe.py --table genz --query "khen đồ ăn"
```

Every row carries `freshness`, `brand_risk`, `use_when`, `avoid_when`, and an `example_line` written
in-register. All 74 rows are recall-graded: slang has a half-life measured in months, so before any
term ships, verify it is still alive in the live comment sections of the actual target audience —
not in a listicle about Gen Z slang, which lags the language by a year.

### The three tiers of brand permission

Which terms a brand may use is a positioning decision, not a taste call, and it goes in the copy
contract before drafting starts:

1. **Native voice** — the brand persona *is* a young person (local F&B, streetwear, a mascot account).
   May use `safe` rows freely in captions and replies, `caution` rows when the platform fits. Still
   never touches `never-in-brand` rows (`trà xanh`, `Tuesday` — both are accusations aimed at a
   person; a brand aiming them at anyone is a PR incident, and using them jokingly still normalises
   the aim).
2. **Borrows in replies only** — most brands. The feed voice stays in the brand's own register;
   slang appears only when replying to a comment that used it first, mirroring the commenter's word
   back. This is how actual people code-switch, which is why it reads as human.
3. **Structure only** — banks, healthcare, B2B, anything where trust is the product. No slang
   vocabulary at all, but the *structures* Gen Z reads as native still apply: short first sentence,
   concrete numbers, no manufactured urgency, replies that answer the actual question. A hospital
   saying `slay` has failed; a hospital answering a pricing comment in one clear sentence within an
   hour has passed the same test.

### The hello-fellow-kids failure

The failure mode has a name because it is that common: a brand using slang correctly by dictionary
definition and still reading as an adult in disguise. The tells, in the order audiences spot them:

- **Density.** Real speech runs one or two slang terms per utterance. Three or more marks the writer
  as performing. The lexicon is for *recognising and selecting*, never for stacking.
- **Expired terms.** `gút chóp` and `bing chilling` are kept in the lexicon as `expired` precisely as
  reference marks: using either in 2026 dates the writer to 2021 more precisely than a copyright
  line would. When a term's freshness reads `aging`, plan its retirement; when `expired`, its only
  legitimate use is self-aware nostalgia, which is a stunt with a one-post budget.
- **Explaining the joke.** Adding a gloss — `khum (tức là không đó nha)` — proves the writer fears
  the audience won't get it, which proves the writer isn't of the audience.
- **Wrong platform register.** Text-speak rows (`khum`, `gòy`, `j z tr`) live in comments, DMs, and
  meme captions. In a carousel headline or on anything printed they read as typos, because there
  they are.

### Misspellings and the diacritic gate

Intentional misspellings are features of the chat register and defects everywhere else. `khum`,
`gòy`, `hông` are correct *as slang* and wrong *as Vietnamese*, and both facts are true at once. The
resolution is scope: on menus, price lines, ingredient lists, and anything legal or transactional,
standard orthography is absolute — a playful caption above a menu photo may say `gòy xong, lên món`,
but the menu itself never does. When a draft deliberately uses chat spelling, declare it in the copy
contract's voice traits; an undeclared `khum` in copy is indistinguishable from a typo, and the
diacritic QA pass is right to flag it.

### The reply test

Before shipping any slang-bearing line, run it through one question: *if a real 20-year-old replied
to this with the same energy, could the brand hold the conversation?* A caption that says `đỉnh nóc
kịch trần` invites replies in kind, and a brand that goes silent or switches to formal Vietnamese in
its own comment section has broadcast that the caption was a costume. Tier-2 brands especially: the
reply thread is where borrowing happens, so it is also where the borrowing is audited.
