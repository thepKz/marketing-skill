...
6. **Cadence.** Before any copy ships, run `scripts/rewrite_human.py --check <file>` on it. It measures what a reread cannot see - sentence-length uniformity, missing landing beats, em-dash density - and matches the calques in `data/translation-tells.csv` that survive translation between Vietnamese and English while staying grammatical. Repair calques first, then rebuild rhythm; the order matters because deleting a connective changes every length measurement after it. Follow `rewrite-human.md`. Do not lower a target to pass, and do not add texture on top of flat prose: variation comes from the content deciding where it needs a beat.
