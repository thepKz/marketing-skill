...
const useCases = {
  ...
  // The step between finishing a file and posting it, which is where a one-shoot budget leaks. This
  // panel exists because the tool behind it refuses to guess: where a vendor page publishes no
  // figure it returns review rather than a pass, so the request string has to ask for the survey
  // rather than for an opinion about ratios.
  channel: {
    vi: { title: 'Một file, 24 vị trí đăng', description: 'Đối chiếu một file đã xuất với spec của 24 vị trí đăng, đọc thẳng từ trang Meta, TikTok, Google Ads, YouTube và Merchant: chỗ nào nhận nguyên, chỗ nào phải cắt lại, chỗ nào từ chối.', deliverables: ['Vị trí nhận nguyên file, và vị trí từ chối', 'Chặn vì phá yêu cầu cứng, hay chỉ lệch khuyến nghị', 'Ô nào nền tảng không công bố con số nào cả', 'Ngày đọc trang, và hạn 90 ngày đọc lại'], request: 'Dùng $marketing-minthep. Tôi có một file [rộng x cao], [số] giây, [dung lượng], định dạng [mp4/jpg]. Chạy scripts/check_channel_spec.py --survey với đúng những số đó, rồi nói tôi vị trí nào nhận nguyên file, vị trí nào phải xuất lại, và vị trí nào trang chủ không công bố số nên phải tự vào xem. Đừng đọc spec từ trong đầu.' },
    en: { title: 'One file, 24 placements', description: 'Check one exported file against the published specs for 24 placements, read off the Meta, TikTok, Google Ads, YouTube and Merchant pages: what takes it untouched, what has to be re-exported, what refuses it.', deliverables: ['The placements that take it, and the ones that refuse', 'A broken requirement against a missed recommendation', 'Which figures the vendor page never published', 'The date each page was read, and a 90-day expiry'], request: 'Use $marketing-minthep. I have one file at [width x height], [n] seconds, [size], format [mp4/jpg]. Run scripts/check_channel_spec.py --survey with those exact numbers, then tell me which placements take it untouched, which need a re-export, and which publish no figure at all so I have to go and look. Do not recall specs from memory.' },
  },
